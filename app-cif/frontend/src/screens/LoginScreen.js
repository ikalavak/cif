...
const SAVED_PASSWORD_KEY = "user_saved_password";
const REMEMBER_ME_KEY = "user_remember_me";
...
export default function LoginScreen({ navigation }) {
    ...
    const loadSavedCredentials = async () => {
      try {
        ...
        if (savedRememberMe === "true") {
          ...
          const savedPassword =
            await SecureStore.getItemAsync(SAVED_PASSWORD_KEY);
        ...
        console.warn("Failed to load saved credentials:", e.message);
      }
    };
  ...
  // --------------------------------------------------
  // Save or Clear Credentials helper
  // --------------------------------------------------
  const handleRememberMeSave = async (emailToSave, passwordToSave) => {
    try {
      if (rememberMe) {
        await SecureStore.setItemAsync(REMEMBER_ME_KEY, "true");
        await SecureStore.setItemAsync(SAVED_EMAIL_KEY, emailToSave);
        await SecureStore.setItemAsync(SAVED_PASSWORD_KEY, passwordToSave);
      } else {
      ...
      console.warn("Error saving login persistence:", e.message);
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await handleRememberMeSave(emailTrimmed, password);
